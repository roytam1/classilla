...
// backbugs and pull up to 1.7
class ClassInfoData
{
public:
    ClassInfoData(nsIClassInfo *aClassInfo, const char *aName) // need to add nsnull to old callers
                                                ...
                                                PRUint32 aAction)
{
    ...
    */
    ...
    rv = SecurityCompareURIs(subjectURI, objectURI, &isSameOrigin);
    NS_ENSURE_SUCCESS(rv, rv);
...
// I decided not to apply most of the backbugs here. Cameron
nsresult
                                        ...
                                        ClassPolicy** result)
{
    ...
    *result = nsnull;
    ...
    aPrincipal->GetSecurityPolicy((void**)&dpolicy); // ??? We set this cache below, should work.
    ...
    {   //-- Look up the relevant domain policy, if any
        ...
        aPrincipal->SetSecurityPolicy((void*)dpolicy); // See above
    }

    ClassPolicy* wildcardPolicy = nsnull;
    
    // Bug 83536 mentions a passed aCachedClassPolicy we can use here, but we
    // don't support that yet. XXX
...
// bug 293671
    ...
    while(PL_strcmp(scheme.get(), "view-source") == 0)
    {
        rv = uri->GetPath(path);
        if (NS_FAILED(rv)) return rv;
        rv = NS_NewURI(getter_AddRefs(uri), path, nsnull, sIOService);
        if (NS_FAILED(rv)) return rv;
        rv = uri->GetScheme(scheme);
        if (NS_FAILED(rv)) return rv;
    }
    ...
    if (PL_strcmp(scheme.get(), "view-source") == 0)
    {
        ...
        //               sIOService);
        rv = NS_NewURI(getter_AddRefs(innerURI), path, nsnull, sIOService);
        if (NS_FAILED(rv)) return rv;
                                         ...
                                         PRUint32 aFlags)
{
    ...
    nsresult rv = NS_NewURI(getter_AddRefs(source), nsDependentCString(aSourceURIStr), nsnull, sIOService);
    NS_ENSURE_SUCCESS(rv, rv);
    nsCOMPtr<nsIURI> target;
    rv = NS_NewURI(getter_AddRefs(target), nsDependentCString(aTargetURIStr), nsnull, sIOService);
    NS_ENSURE_SUCCESS(rv, rv);
    ...
    *result = PR_FALSE; 
    ...
    if (globalObjTreeItem) 
    {
        ...
        do 
        {
            ...
            // we don't have .swap (see bug 83536)
            if (treeItem) 
            {
...
// this is a scary one!
            ...
            if (!JS_GetReservedSlot(cx, obj, 2, &v))
                return NS_ERROR_FAILURE;
            //v = DOUBLE_TO_JSVAL(v); // convert from the rooted double to a base jsval
            if (JSVAL_IS_VOID(v))
                return doGetObjectPrincipal(cx, obj, result);
...
// Shouldn't be needed anymore
    ...
    if (!ssm)
    	return NS_ERROR_FAILURE;
    // GetSystemPrincipal returns a refcounted object. Trying to use an
    // nsCOMPtr here for belt-and-suspenders actually crashes the browser.
...
nsScriptSecurityManager::SavePrincipal(nsIPrincipal* aToSave)
{
    ...
    nsresult rv = persistent->GetPreferences(getter_Copies(idPrefName),
                                    getter_Copies(id),
...
nsScriptSecurityManager::CanCreateWrapper(JSContext *cx,
                                          ...
                                          void **aPolicy)
{
#if DEBUG_CAPS_CanCreateWrapper
...
nsScriptSecurityManager::GetScriptSecurityManager()
{
    if (!gScriptSecMan)
    {
        ...
        rv = sXPConnect->SetDefaultSecurityManager(
...
//    mPolicyPrefsChanged = PR_FALSE; // bug 217967
    ...
    rv = sXPConnect->ClearAllWrappedNativeSecurityPolicies();
    if (NS_FAILED(rv)) return rv;
