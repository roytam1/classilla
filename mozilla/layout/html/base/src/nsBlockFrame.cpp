 ...
 * ***** END LICENSE BLOCK ***** */
 ...
 /* CLASSILLA CLASSILLA pull ups attempted to 1.7.1
    ones we could not accomplish are marked with "we lack" */
...
// our version of nsBlockFrame::GetType() const
NS_IMETHODIMP
...
// equivalent of GetAdditionalChildListName(
NS_IMETHODIMP
...
// converted to CalculateContainingBlockSizeForAbsolutes later on.
static void
                         ...
                         nscoord&                 aContainingBlockHeight)
{
  ...
  if (aReflowState.mStyleDisplay->IsAbsolutelyPositioned() ||
    ...
    // bug 282754 modified for 1.3.1
    //if (frame->GetParent()->GetContent() == frame->GetContent()) {
    nsIFrame *frame = aReflowState.frame;
    nsCOMPtr<nsIContent> frameContent;
    nsIFrame *parentFrame;
    nsIContent *parentFrameContent;
    frame->GetParent(&parentFrame);
    frame->GetContent(getter_AddRefs(frameContent));
    parentFrame->GetContent(&parentFrameContent);
    
    if(parentFrameContent == frameContent) {
    ...
    //while (aLastRS->parentReflowState->frame->GetContent() == frame->GetContent()) {
    while(1) {
      nsIContent *prspContent;
      aLastRS->parentReflowState->frame->GetContent(&prspContent);
      if (prspContent == frameContent)
      	break;
    ...
    if (aLastRS != &aReflowState) {
      ...
      if (aLastRS->mComputedWidth != NS_UNCONSTRAINEDSIZE) {
        ...
        aContainingBlockWidth = aLastRS->mComputedWidth;
      }
      if (aLastRS->mComputedHeight != NS_UNCONSTRAINEDSIZE) {
        //cbSize.height = aLastRS->mComputedHeight;
        aContainingBlockHeight = aLastRS->mComputedHeight;
      }
    }
    }
    // end bug
...
// this totally doesn't work.
  ...
  // ... bug 281267 (and bug 299742): targeted only at
  // absolute frames and we can skip ReflowDirtyLines().
  // however, we don't yet have ReflowingAbsolutesOnly(), though we do have much
  // of the same logic in nsAbsoluteContainingBlock.cpp::IncrementalReflow(). -- Cameron
  PRBool needToReflowLines = aMetrics.mComputeMEW ||
    ...
    ((NS_BLOCK_HAS_CLEAR_CHILDREN & mState) && //((GetStateBits() & NS_BLOCK_HAS_CLEAR_CHILDREN) &&
    ...
    !(NS_BLOCK_SPACE_MGR & mState); // !(GetStateBits() & NS_BLOCK_SPACE_MGR);
  // end bug

  
  if (mAbsoluteContainer.HasAbsoluteFrames() &&
  	  !needToReflowLines && // bug 281267
...
//                           NS_BLOCK_MARGIN_ROOT & mState); // bug 209694
                           //aReflowState.mFlags.mHasClearance || // bug 292295
  ...
  BuildFloaterList(); // equivalent to BuildFloatList()
  
  // Compute our final size
  ComputeFinalSize(aReflowState, state, aMetrics);
  StoreOverflow(aPresContext, aMetrics); // bug 197581
  ...
  // CheckInvalidateBorder
  CheckInvalidateSizeChange(aPresContext, aMetrics, aReflowState); // bug 197065
...
PRBool
nsBlockFrame::CheckForCollapsedBottomMarginFromClearanceLine(nsIPresContext* aPresContext)
{ 
  for (line_iterator line = --end_lines(), begin = begin_lines();
       line != begin; --line) {
    if (line->mBounds.height != 0 ||
//     || !line->IsEmpty()) {
       !IsLineEmpty(aPresContext, line))
         return PR_FALSE;
                               ...
                               nsHTMLReflowMetrics&     aMetrics)
{
  ...
  // Compute final width
  // 1.7 calls this maxElementWidth. it is the same.
  nscoord maxWidth = 0;
    ...
    if (aState.GetFlag(BRS_COMPUTEMAXELEMENTWIDTH)) {
      ...
      if (aReflowState.mStylePosition->mWidth.GetUnit() == eStyleUnit_Percent) {
        ...
        maxWidth = aState.mMaxElementWidth +
         ...
         // fall through to original state ... now we CAN'T shrink any smaller.
         maxWidth = aMetrics.width;
       }
     }
  }
...
// pull up to 1.8.1
// DISABLED PULL UP (old code is front)
#if(1)
    ...
    if (NS_UNCONSTRAINEDSIZE != aReflowState.mComputedMaxWidth)
    	computedWidth = PR_MIN(computedWidth, aReflowState.mComputedMaxWidth);
    computedWidth = PR_MAX(computedWidth, aReflowState.mComputedMinWidth);
    computedWidth += borderPadding.left + borderPadding.right;
    if (aState.GetFlag(BRS_COMPUTEMAXELEMENTWIDTH)) {
    	nscoord computedMinWidth = aReflowState.mComputedMinWidth +
        	borderPadding.left + borderPadding.right;
        	
		if (maxWidth < computedMinWidth && // GetStylePosition()->
			aReflowState.mStylePosition->mMinWidth.GetUnit() != eStyleUnit_Percent)
		  maxWidth = computedMinWidth;
	}
#endif
...
// end bug
// pull up to 1.8.1
// I'd like to call ReflowDirtyLines a LOT less.
#if(1)
// BACKED OUT
...
#warning this may not work right yet
      ...
      for (const nsHTMLReflowState *prs = aReflowState.parentReflowState;
        ...
        nsCOMPtr<nsIAtom> fType;
        prs->frame->GetFrameType(getter_AddRefs(fType));
           
        /* if (prs->frame->GetType() == nsLayoutAtoms::blockFrame ||
            prs->frame->GetType() == nsLayoutAtoms::areaFrame) { */
        if (fType == nsLayoutAtoms::blockFrame || fType == nsLayoutAtoms::areaFrame) {
          rewrap = PR_FALSE;
          break;
        }
      }
...
// bug 209694
  ...
  if (!aState.GetFlag(BRS_ISBOTTOMMARGINROOT)) {
    ...
    if (CheckForCollapsedBottomMarginFromClearanceLine(aState.mPresContext)) { // modified 1.3
...
// end bug
      if (mPrevInFlow) {
        ...
        for (nsIFrame* prev = mPrevInFlow; prev; prev->GetPrevInFlow(&prev)) {
          ...
          //aMetrics.height += borderPadding.top; // now bug 174688 is fixed and here it is:
          nscoord contentHeight = rect.height; // prev->GetRect().height; // modified for 1.3
          if (prev == mPrevInFlow) {
        ...
        computedHeightLeftOver = PR_MAX(0, computedHeightLeftOver); // bug 174688
      }
// bug 174688
#if(0)
...
// end bug
      ...
      // This overrides 209694 with 174688
      //aMetrics.height = aState.mY + nonCarriedOutVerticalMargin; // +nCOV from 209694
    ...
    aMetrics.mCarriedOutBottomMargin.Zero(); // aState.mPrevBottomMargin.Zero(); // 209694
  }
  else {
    nscoord autoHeight = aState.mY + nonCarriedOutVerticalMargin; // 209694
    ...
    // bug 295309 with extensions for 1.3.
#define BUG295309
#ifndef BUG295309
    ...
    // Apply min/max values
    // equivalent of aReflowState.ApplyMinMaxConstraints(
    // well ... not really. that's why we're ...
...
#warning this might not work right yet
	...
	autoHeight = PR_MAX(autoHeight, aReflowState.mComputedMinHeight);
#endif
	// end pull up
	
#ifdef BUG295309    
    // bug 295309 plus backbugs from 1.8
...
// bug 209694 (this was moved up earlier)
#if(0)
  // Return bottom margin information
  // rbs says he hit this assertion occasionally (see bug 86947), so
  // just set the margin to zero and we'll figure out why later
...
/* OPTIMIZE THE SHET OUT OF ME BABY */
void
                                              ...
                                              nsIFrame *aPrevInFlow)
{
  ...
  do {
    ...
    --aLine;
    if (aLine->GetBreakTypeAfter() == NS_STYLE_CLEAR_LINE) // After from bug 209694
      break;
...
// this is unique to 1.3. we really need it, because IsEmpty() is hell.
PRBool
...
// bug 209694 has nothing about wrapping in it. maybe we should take this out.
// so we will, since 1.7 doesn't have it either.
         ...
         ++line)
    {
      ...
      if (line->IsBlock() ||
          ...
          line->HasFloaters() ||
          (// wrapping && // pull up to 1.7
           ((line != mLines.back() && !line->HasBreakAfter()) || // bug 209694
      ...
      if (gNoisyReflow && !line->IsDirty()) { // && wrapping) {
        ...
        printf("skipped: line=%p next=%p %s %s %s%s%s breakTypeBefore/After=%d/%d xmost=%d\n",
           ...
           line->HasFloaters() ? "has-floaters " : "", // equivalent of ->HasFloats()
...
// we do not have static PRUint8 CombineBreakType(...
// bug 209694 modified for 1.3
static PRBool LineHasClear(nsLineBox* aLine) {
  nsFrameState frameState;
  aLine->mFirstChild->GetFrameState(&frameState);
  return aLine->GetBreakTypeBefore() || aLine->HasFloatBreakAfter()
    || (aLine->IsBlock() && (frameState & NS_BLOCK_HAS_CLEAR_CHILDREN));
}
...
/* THIS FUNCTION IS EXPENSIVE TO CALL.
   WE NEED TO OPTIMIZE THE SHIZZLE OUT OF THIS. -- Cameron Kaiser */
...
nsBlockFrame::ReflowDirtyLines(nsBlockReflowState& aState)
{
  ...
  PRBool foundAnyClears = PR_FALSE; // bug 209694
  ...
  PRBool  incrementalReflow = aState.mReflowState.reason ==
                              ...
                              aState.mReflowState.reason ==
                              eReflowReason_Dirty ||
                              aState.mReflowState.reason == eReflowReason_Resize; // bug 207477
  ...
  // We save up information about BR-clearance here // bug 209694
...
// pull up to 1.8.1
// GET THIS THE HELL OUT OF THE REFLOW LOOP! ARGH! -- Cameron
#if(0)
    // If we're supposed to update our maximum width, then we'll also need to
    ...
    // the line.
    // bug 206516 implies this is no longer in the line iterator, but we have no
    // ::DirtyLinesWithDirtyContinuations() in 1.3.1 yet.
    // before doing this check (it's O(N^2) as written now). (added for 209694 context)
...
// bug 209694
    ...
    // modified for 1.3 for GetStateBits()
    nsFrameState frameState;
    line->mFirstChild->GetFrameState(&frameState);
    
    if (line->IsBlock() &&
        (frameState & NS_BLOCK_HAS_CLEAR_CHILDREN)) {
      line->MarkDirty();
    }
...
// pull up to 1.8.1
#if(1)
// old code is reinstated. with the pull up in place, large gaps got
// introduced in certain table based pages, most notably Amazon and
// our own http://www.floodgap.com/retrobits/tomy/pyuuta/
    ...
    if (needToRecoverState &&
        ...
        // if (floatBreakType ... was not in 1.3 (bug 209694)
...
#warning this does not work yet.
    ...
    if (!line->IsDirty()) {
      // See if there's any reflow damage that requires that we mark the
      // line dirty.
      PropagateFloaterDamage(aState, line, deltaY); // PropagateFloatDamage(...
    }
...
// end pullup
    ...
    // end pull up  
      ...
      rv = ReflowLine(aState, line, &keepGoing, incrementalReflow); // forceInvalidate);
      if (NS_FAILED(rv)) {
        return rv;
      }
      /* pull up to 1.7
      if (line->HasFloaters()) { // HasFloats()
...
#if(1)
// old code is reinstated. with the new code in the else block,
// when a line would wrap, it would add padding to the next line break
...
// this is the 1.8.1 code that does not fully work yet.
      ...
      // refactored a bit since we have to do some expensive calculations for 1.3.1.
      if (line.next() != end_lines()) {
        PRBool maybeWasEmpty = oldY == oldYMost;
        PRBool isEmpty = line->mBounds.height == 0;
        // && line->IsEmpty(); // line->CachedIsEmpty(); // we do that below.
        if (maybeReflowingForFirstTime /*1*/ ||
            (isEmpty || maybeWasEmpty) /*2/3/4*/) {
            // let's only do the IsEmpty computation if we really, really have to.
            // (unfortunately by this point we do)
            if (IsLineEmpty(aState.mPresContext, line))
              line.next()->MarkPreviousMarginDirty();
          // since it's marked dirty, nobody will care about |deltaY|
        }
      }
...
// end pull up
      ...
      lastLineMovedUp = deltaY < 0; // pull up to 1.8.1
      ...
      // and also because our final height may depend on it. If the (bug 209694)
      ...
      // modified for 1.3
      // we have no CachedIsEmpty()
      if (line->IsBlock() || !IsLineEmpty(aState.mPresContext, line)) { //|| !line->IsEmpty()) {
        aState.mY = line->mBounds.YMost();
        // no floatBreakType line in 1.3
...
// end of nasty line iterator loop
  ...
  while (keepGoing && (nsnull != aState.mNextInFlow)) {
    ...
    while (line != end_lines()) {
      rv = ReflowLine(aState, line, &keepGoing,
                      incrementalReflow /* force invalidate */); // consequence of bug 207477
      if (NS_FAILED(rv)) {
        return rv;
      }
      if (!keepGoing) {
...
// bug 209694
  ...
  if (foundAnyClears) {
    ...
    mState |= NS_BLOCK_HAS_CLEAR_CHILDREN;
  } else {
...
//if (oldCombinedArea.TopLeft() != lineCombinedArea.TopLeft() || oldBounds.TopLeft() != newBounds.TopLeft())
...
/*          if (!dirtyRect.IsEmpty()) {
        ...
        // No, it doesn't in 1.3
#define S_Invalidate(x) if(!x.IsEmpty()) Invalidate(aState.mPresContext, x);
        S_Invalidate(boundsVStrip);
        S_Invalidate(boundsHStrip);
        S_Invalidate(combinedAreaVStrip);
        S_Invalidate(combinedAreaHStrip);        
      }
    }
  }
  else {
  ...
  if (fromLine->IsBlock()) {
    ...
    if (0 != --fromLineChildCount) {
      ...
      if (aDamageDeletedLines && !fromLine->mBounds.IsEmpty()) {
        Invalidate(aState.mPresContext, 
        	fromLine->mBounds);
      }
               ...
               nsIFrame*       aFrame)
{
  ...
  if (view)
//    nsContainerFrame::SyncFrameViewAfterReflow(aPresContext, aFrame, view, nsnull); // bug 79315
    nsContainerFrame::PositionFrameView(aPresContext, aFrame); // bug 79315
  else // pull up 1.7
...
// this was moved into nsLayoutUtils by bug 209694
#if(0)
                               ...
                               PRBool* aKeepReflowGoing)
{
  ...
  frame->GetPrevInFlow(&framePrevInFlow); // needed for ->GetPrevinFlow() below
// bug 209694
#if (0)
  ...
  if (NS_STYLE_CLEAR_NONE != aState.mFloaterBreakType) {
    breakType = nsLayoutUtils::CombineBreakType(breakType, aState.mFloaterBreakType);
...
// bug 209694
  ...
  PRBool applyTopMargin =
    !framePrevInFlow // !frame->GetPrevInFlow() 
    && ShouldApplyTopMargin(aState, aLine);
...
// bug 209694
    ...
    aState.mPrevBottomMargin = oldPrevBottomMargin; // perhaps not needed
      ...
      nsBlockReflowContext::ComputeCollapsedTopMargin(aState.mPresContext, reflowState,
...
// end bug
    ...
    // space. This undone as soon as the horizontal margins are computed. (bug 209694)
      topMargin = aState.mPrevBottomMargin.get();
...
// end bug    
  ...
  // after landing this bug, Here aState.mY is the top border-edge of the block.
...
//  nsReflowStatus frameReflowStatus=NS_FRAME_COMPLETE; // NO! NOT YET! bug 209694
  nsMargin computedOffsets;
  // construct the html reflow state for the block. ReflowBlock 
  // will initialize it 
  nsHTMLReflowState blockHtmlRS(aState.mPresContext, aState.mReflowState, frame, 
                                nsSize(availSpace.width, availSpace.height), 
                                aState.mReflowState.reason, //PR_FALSE); // bug 209694
                                PR_TRUE);
// bug 209694
    blockHtmlRS.mFlags.mHasClearance = aLine->HasClearance();
    
    // bug 105520
    if (!mayNeedRetry && isImpacted && display->IsBlockLevel() &&
    	splitType != NS_FRAME_SPLITTABLE_NON_RECTANGULAR &&
    	!display->IsFloating() && !clearanceFrame &&
    	breakType != NS_STYLE_CLEAR_LEFT_AND_RIGHT)
    		mayNeedRetry = PR_TRUE;
    // end bug
...
// end bug                                
  rv = brc.ReflowBlock(availSpace, applyTopMargin, aState.mPrevBottomMargin,
                       clearance, // bug 209694
                       aState.IsAdjacentWithTop(), computedOffsets, 
                       blockHtmlRS, frameReflowStatus);

// this is not in bug 209694. remove?
// yes. it was removed by bug 207477
...
// bug 105520
    // If a non-floating block-level element that doesn't wrap its lines around the
    // float doesn't fit on a line that is impacted by floats then reflow once
    // more, now clearing the floats.
    if (isImpacted && display->IsBlockLevel() &&
        splitType != NS_FRAME_SPLITTABLE_NON_RECTANGULAR &&
        !display->IsFloating() && brc.GetMetrics().width > availSpace.width &&
        !clearanceFrame) {
      breakType = NS_STYLE_CLEAR_LEFT_AND_RIGHT;
      treatWithClearance = PR_TRUE;
      applyTopMargin = PR_TRUE;
      clearanceFrame = frame;
      // Fall through into the next if-block.
    }
// end bug
...
// end bug
  ...
  if (NS_INLINE_IS_BREAK_BEFORE(frameReflowStatus)) {
    ...
    *aKeepReflowGoing = brc.PlaceBlock(blockHtmlRS, isAdjacentWithTop,
                                       aLine->HasClearance(), // bug 209694
                                       computedOffsets, collapsedBottomMargin,
                                       aLine->mBounds, combinedArea);
                                       
    //aLine->SetCarriedOutBottomMargin(collapsedBottomMargin);
    // bug 209694 has this as part of an iteration loop
    // ... because bug 257612 gave it to us, and here it is.
                                ...
                                PRUint8* aLineReflowStatus)
{
  ...
  *aLineReflowStatus = LINE_REFLOW_OK;
  ...
  aLine->SetBreakTypeAfter(NS_STYLE_CLEAR_NONE); // After from bug 209694
  if (NS_INLINE_IS_BREAK(frameReflowStatus) || 
    ...
    *aLineReflowStatus = LINE_REFLOW_STOP;
    ...
    if (NS_INLINE_IS_BREAK_BEFORE(frameReflowStatus)) {
      ...
      aLine->SetBreakTypeAfter(breakType); // bug 209694
      if (NS_FRAME_IS_NOT_COMPLETE(frameReflowStatus)) {
...
// removed by 1.7
  ...
  if (aLine->HasFloaters()) {
    ...
    lineCombinedArea.UnionRect(aState.mFloaterCombinedArea, lineCombinedArea); // 1.7
...
// This is the 1.3 equialent of "Out-of-flow floats need to be reparented too."
...
*/
  ...
  while (newFrame) {
    ...
    nsCOMPtr<nsIAtom> fType;
    if(aPrevSibling)
    	aPrevSibling->GetFrameType(getter_AddRefs(fType));
...
////////////////////////////////////////////////////////////////////////
// Floater support or as 1.8 calls it, // Float support
// bug 216303 removes this function. leaving it here just in case.
// bug 209694
// changed to nsIPresContext for 1.3
static void InitReflowStateForFloat(nsHTMLReflowState* aState, nsIPresContext* aPresContext)
{
  ...
  const nsStylePosition *position;
  aState->frame->GetStyleData(eStyleStruct_Position, (const nsStyleStruct *&)position); // modified for 1.3
...
// this is the same as ReflowFloat()
nsresult
...
// this is not in 1.8.1.
...
#endif
  // Reflow the floater.
  nsIFrame* floater = aPlaceholder->GetOutOfFlowFrame(); // floater is our floatFrame.
  ...
  // initialize it. // not and set its reason.
...
// bug 216303
  ...
  InitReflowStateForFloat(&floaterRS, aState.mPresContext); // bug 209694 for 1.3
#endif
                              aState.mReflowState.reason);
...
// bug 209694 converted for 1.3
// LOCKUP
  nsIFrame *clearanceFrame = nsnull;
  nsresult rv;
  int loopthru = 0; // this is a KLUDGE FOR THE LOCKUP 209694
  const int maxiter = 5; // who knows what this should be? 64 clearly too high. 17 even iffy.
  do {
...
//                                aComputedOffsetsResult, floaterRS, aReflowStatus);
    PRBool mayNeedRetry = PR_FALSE;
    floaterRS.mDiscoveredClearance = nsnull; // pull up to 1.8.1
    // Only first in flow gets a top margin.
    // if (!floatFrame->GetPrevInFlow()) {
    nsIFrame *foo;
    floater->GetPrevInFlow(&foo);
    if (!foo) {
    	nsBlockReflowContext::ComputeCollapsedTopMargin(aState.mPresContext, floaterRS, &margin,
                                                    clearanceFrame, &mayNeedRetry);
    
    	if (mayNeedRetry && !clearanceFrame) {
      		floaterRS.mDiscoveredClearance = &clearanceFrame;
      		...
      		floaterRS.mDiscoveredClearance = nsnull; // do we need this?
    	}
    }
    
    // tried figuring out where rv is actually returned false and I have no fricking idea.
    // nothing is clearly setting clearanceFrame. -- a frustrated CK
    rv = brc.ReflowBlock(availSpace, PR_TRUE, margin,
                         0, isAdjacentWithTop, 
                                 // aFloatCache->mOffsets, converted for 1.3
                                 aComputedOffsetsResult,
                                 floaterRS,
                                 aReflowStatus);
  } while (NS_SUCCEEDED(rv) && clearanceFrame && ++loopthru < maxiter);
// end bug
...
//      nsCollapsingMargin marginMEW;
      ...
      nsHTMLReflowState redoFloaterRS(aState.mPresContext, aState.mReflowState, floater, 
                                      nsSize(availSpace.width, availSpace.height), 
                                      aState.mReflowState.reason); // , PR_FALSE); // bug 216303

// bug 209694     
// LOCKUP  KLUDGE 209694                               
//      rv = brc.ReflowBlock(availSpace, PR_TRUE, marginMEW, isAdjacentWithTop, 
//                           aComputedOffsetsResult, redoFloaterRS, aReflowStatus);
//      InitReflowStateForFloat(&redoFloaterRS, aState.mPresContext); // bug 216303
      ...
      do {
        ...
        PRBool mayNeedRetry = PR_FALSE;
        nsBlockReflowContext::ComputeCollapsedTopMargin(aState.mPresContext,
            redoFloaterRS, &marginMEW, clearanceFrame, &mayNeedRetry);
        
        if (mayNeedRetry && !clearanceFrame) { // modified for 1.3
          redoFloaterRS.mDiscoveredClearance = &clearanceFrame;
          // We don't need to push the space manager state because the the block has its own
          // space manager that will be destroyed and recreated
        } else { 
          redoFloaterRS.mDiscoveredClearance = nsnull;
        }
...
//  }
  ...
  if (aState.GetFlag(BRS_COMPUTEMAXELEMENTWIDTH)) { // computeMaxElementWidth) {
    nscoord mew = brc.GetMaxElementWidth();
    mew += aMarginResult.left + aMarginResult.right;
    aState.StoreMaxElementWidth(floater, mew); // this is commented out in 1.7 pull up
...
// bug 209694
        ...
        if (endLine->HasFloatBreakAfter()) {
          aState.mFloaterBreakType = endLine->GetBreakTypeAfter(); // we use FloaterBreak in 1.3
